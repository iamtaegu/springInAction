...
$.each($('.tbl-guide tbody tr'), function(i){
    ...
    if(href == '#'){
        ...
        }).css({'cursor' : 'default', 'text-decoration' : 'none', 'color' : '#999'});

        link.addClass('not-yet');

        return;

    }

    var divide = that.find('td').eq(0).text();

    if(divide != ''){

        that.addClass('devide');

    }

    that.find('.link').append(' <a class="link-blank" href="'+ href +'" target="_blank" title="새창으로 열기">=</a>');

    that.find('.link').append('<span class="file-name">' + file + '</span>');

    var link = that.find('a:not(.link-blank)');
    ...
    var publish = that.find('td').eq(3).text();

    if(publish == 'o' || publish == 'O'){

        that.find('.file-name').addClass('publish');

    }

});

$('.payboock-tab-btn a').bind('click', function(){

    var that = $(this);

    var targetAll = $('.section');

    targetAll.hide();

    $(that.attr('href')).show();

    $('.payboock-tab-btn a').removeClass('current');
...
$.each($('.tbl-guide'), function(i){

    if(i<5) return;

    var that = $(this);

    var tr = that.find('tbody tr:not(".no-count")');

    var bar = that.parent().prev('h3');

    var count = tr.find('a.not-yet'); // 링크가 있는

    var endCount = 0; // 완료일이 적힌
    ...
    count = tr.length - count.length;

    $('<span class="total"></span>').appendTo(bar).text('('+ count + '/' + tr.length +')');
...
document.querySelectorAll('code').forEach(function(element){
    ...
    element.innerHTML = element.innerHTML.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/>/g, '&gt;').replace(/"/g, '&quot').replace(/'/g, '&#039');
